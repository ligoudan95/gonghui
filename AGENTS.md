...
- 当前阶段：DEMO 开发中——M0/M1/M2 已完成（**M1 硬验收门 2026-09-25 用户试玩通过**，验收时 selfcheck 307 用例全绿；**M2 事件与检定 2026-09-25 用户试玩验收通过**，验收口径 selfcheck 370 用例+92 资源全绿）；**M3 探索层三批实施完成＋内部质检 43 项全修闭环（2026-09-25 用户拍板「全部按推荐」，selfcheck 432→447 用例+121 资源全绿；P1 占位出征面板+q_lair_purge 落 1 行/P2 拆事件演示入口留 event_screen 至 M4/P3 总图 ≤15×15；tp_old_well 移矿洞深处/q_lair_purge 人力改回 3-4/自由探索可交付+B 出口战不计 CLEAR 判据+失败隐藏收获行+预览豁免+回城存档归 M4）＋第四轮全库审计 54 项全修闭环（2026-09-26，selfcheck 527 用例+121 资源全绿；六职业技能验证网 31 用例；缩放失活 W3-01/移动致死软锁 W2-1/单点 B 出口锚点 W2-2等重点修复；W5-1 随机战斗关闭拍板落注三案+encw_mine 0.0；校验 V-M3 新增三条+八处扩展案 16 §2.6），🔶 待用户试玩验收**；下一步 M3 验收→M4 经营层（待 M3 验收通过后出架构方案）

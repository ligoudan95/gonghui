...
- 当前阶段：DEMO 开发中——M0/M1 已完成（**M1 硬验收门 2026-09-25 用户试玩通过**，验收时 selfcheck 307 用例全绿）；**M2 事件与检定三批实施完成（含 M1 遗留 first_strike 补做）＋内部质检 40 项全修闭环（2026-09-25 用户拍板「全部按推荐」，selfcheck 370 用例+92 资源全绿；战后续跑断链/四档反馈接线/倒地过滤/D20 门禁等重点修复），🔶 待用户试玩验收**；下一步 M2 验收→M3 探索层（待 M2 验收通过后出架构方案）

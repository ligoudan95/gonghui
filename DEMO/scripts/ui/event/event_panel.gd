...
func show_settled(grade: int, text: String, reward_text: String) -> void:
	## 呈现结算视图：四档反馈条（色标签）+ 结算文本 + 数值反馈行 + 继续钮
	...
	_result_label.text = text + ("\n[b]" + reward_text + "[/b]" if not reward_text.is_empty() else "")
	_continue_button.visible = true

func show_battle_intro(view: EventRunner.EventView, reward_text: String) -> void:
	## 呈现 B 出口战前演出视图（E3-06 拍板 A：先播再战）——叙述+结算文本+
	## 四档反馈（若经检定）+ 数值反馈行 +「进入战斗」钮
	...
	_result_label.text = view.narrative \
			+ ("\n[b]" + reward_text + "[/b]" if not reward_text.is_empty() else "")
